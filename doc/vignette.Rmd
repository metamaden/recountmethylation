---
title: 'Vignette for ''recountmethylation'' package: basic data retrieval, queries,
...
```{r, echo = T}
dbn = "remethdb_test.h5" # path to h5 db
rhdf5::h5ls(dbn)
```
...
# Data queries
DNA methylation signal data can be retrieved in one of two ways using the `recountmethylation::getrg()` function. First, data can be returned as an object of class `RGChannelSet`, a type of `SummarizedExperiment` object, by setting the `dat.type` argument to 'se'. 

```{r, echo = T}
# make df list from h5 db query
se.rgi = getrg(gsmv = gsmvi, cgv = cgvi, dbn = dbn, dat.type = "se")
dim(se.rgi)
dim(pData(se.rgi))
dim(getRed(se.rgi))
dim(getBeta(se.rgi))
...
for(c in colnames(rgdl)){
  evalv = c(evalv, identical(as.numeric(getRed(rgdl[,c])), as.numeric(getRed(se.rgi[,c]))))
  evalv = c(evalv, identical(as.numeric(getGreen(rgdl[,c])), as.numeric(getGreen(se.rgi[,c]))))
}
```
...
We can compare raw Beta-values to noob-normalized Beta-values as follows:
