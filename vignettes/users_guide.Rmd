...
The `HDF5-SummarizedExperiment` files will likely be the most convenient when working from an R session. For workflows requiring an HDF5 database, an `h5` database file containing red and green channel signals is also provided and supported by functions in `recountmethylation`.

## File downloads

The latest compilation files can be downloaded from the data server with the `get_rmdl` function. In `get_rmdl`, specify the arguments `which.class` (either rg, gm, gr, or test) and `which.type` (either "h5se" or "h5") to automatically download the latest versions of files. The filename argument `fn` may also be specified to download a specific file. 

Setting `show.files = TRUE` returns server file details. Download the test datasets and print the server files table to console as follows.

```{r}
st <- get_rmdl(show.files = TRUE, download = FALSE)
```
...
dnpath <- get_rmdl(which.class = "test", which.type = "h5se")
gr.h5se <- loadHDF5SummarizedExperiment(dnpath)
```
...
```{r fpath_h5}
library(rhdf5)
dbpath <- get_rmdl(which.class = "test", which.type = "h5")
h5ls(dbpath)
```
