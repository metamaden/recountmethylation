...
Use the file path to read data into an `RGChannelSet` with the `getrg` function. Set `all.gsm = TRUE` to retrieve signals for all available GSM IDs (columns).

```{r}
h5.rg <- getrg(dbn = h5.test, all.gsm = TRUE)
```

To avoid exhausting active memory with the full-sized `h5` dataset, provide either `gsmv` or `cgv` to `getrg`, and set either `all.cg` or `all.gsm` to `FLASE` (see `?getrg` for details).
