...
library(basilisk); library(reticulate); library(HDF5Array); library(ggplot2)
...
#md.fpath <- file.path(si.dapth, "mdf_sitest.rda")
# load objects
#bval <- get(load(bval.fpath))
#md <- get(load(md.fpath))
# get(load("si_vignette_env.Rda"))
bval.fpath <- "bval_100.csv"
md.fpath <- "mdf_sitest.rda"
fhtable.fpath <- "bval_100_fh10.csv"
md <- get(load(md.fpath))
bval <- read.csv(bval.fpath, row.names = 1)
fhtable <- read.csv(fhtable.fpath, row.names = 1, header = F)
source("dnam_search_index.R")
```
...
First, save DNAm fractions (Beta-values) from a `SummarizedExperiment` object, ensuring sample data is in rows and probes are in columns.
...
Next, call `get_fh()` to perform feature hashing on the DNAm data. For this example, we set the reduced dimensions using `ndim = 100`.
...
si.fname <- paste0(si.fname.str, ".pickle")
sidict.fname <- paste0(si.fname.str, "_dict.pickle")
make_si(fh_csv_fpath = fhtable.fpath, si_fname = si.fname, si_dict_fname = sidict.fname)
```
...
```{r, eval = T}
gsei.str <- "GSE67393"
gsmv.gsei <- rownames(md[md$gse==gsei.str,])
which.gsmv <- which(rownames(bval) %in% gsmv.gsei)
query.gsmv <- rownames(bval[which.gsmv,])[seq(10)]
```
...
```{r, eval = T}
dfnn <- query_si(sample_idv = query.gsmv, fh_csv_fname = fhtable.fpath,
                 index_dict_fname = sidict.fname, lkval = lkval)
```
