...
This vignette makes use of the sample metadata and 2 of the available `HDF5-SummarizedExperiment` datasets Consult the `users_guide` vignette for details about data and access. For this vignette, access the versioned metadata and load the DNAm datasets as follows.
...
Next, prepare genome region heatmaps with 3 helper functions. These will tabulate probe abundances by region and use the probe Beta-value means to calculate the means (left heatmap) and variances (right heatmap) by genome region type. 
