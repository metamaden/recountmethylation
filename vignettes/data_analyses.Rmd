...
This vignette describes analysis steps in full, but with limited evaluations to ensure package build success. Code has been written with the intention of extensibility to new and larger comparator groups. Unabridged code and helper function definitions to reproduce these examples are contained in the `data_analyses.R` script.

## Data availability

This vignette makes use of the sample metadata and 2 of the available `HDF5-SummarizedExperiment` compilations. Consult the `users_guide` vignette for details about data and access. For this vignette, access the versioned metadata and load the DNAm compilation objects as follows.
...
```{r, eval = TRUE, fig.width = 4, fig.height = 3.8}
...
```{r, eval = TRUE, fig.width = 5, fig.height = 4.5}
...
Next, prepare to calcualte log2 median signals. To manage data in active memory, process it in smaller units or blocks. Using the `get_blocks` helper function, assign sample indices to blocks of size 1,000 using the `bsize` argument.
...
Evaluate signal patterns across storage type using plots using the ggplot2 package. First, make a 2d scatter plot of methylated and unmethylated signals using the `geom_point` function. Color by storage type with the `scale_color_manual` function.
...
Next, make separate violin plots for signals and groups using the `geom_violin` function. Draw horizontal median lines by setting the `draw_quantiles` argument to 0.5.
...
# Example 3: Identify and analyze tissue-specific probes with the highest variances

This example describes variance analyses in liver and adipose, 2 of the 7 tissues analyzed in the manuscript @maden_human_2020. This includes a quality assessment, study ID linear adjustment of DNAm fractions, ANOVA-based and probe filtering, 2-step variance analyses, and results plots.
...
kable(sst.tvar, align = "c")
```

## Calculate log2 methylated and unmethylated signal medians

Subset the `MethylSet` compilation, then append the `sgroup` variable from `mdf` and map the object to the genome using the `mapToGenome` function from the `minfi` package.
...
Next, prepare genome region heatmaps with 3 helper functions as shown. First, get island and gene annotation groups using `get_cga`. Next, get region-specific DNAm summaries with `hmsets` using a minimum region coverage of 2. This means values are only calculated for regions with least 2 probes. Regions with less are assigned "NA" and would be grayed out. Make the 2 plots objects with `hmplots()`, which wraps the `geom_tile` ggplot2 function. Display plots horizontally with `grid.arrange`.
...
The heatmaps are colored blue, white, red for low, intermediate, and high DNAm fraction means and variances. Cell numbers show probe region quantities, which are identical for both plots.
