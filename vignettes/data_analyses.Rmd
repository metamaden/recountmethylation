...
Next, make separate violin plots, with horizontal bars at distribution medians, for each signal and storage type.
...
Subset the `MethylSet` compilation, append `sgroup`, and map it to the genome.
...
For quality assessment, calculate log2 median signals. As above, process the data in blocks to track progress.
...
lqc <- list("l2med.meth" = l2med.meth, "l2med.unmeth" = l2med.unmeth, 
            "gsmv" = ms$gsm)
```
...
Make violin plots, with horizontal bars at distribution medians, for DNAm of each probe set using the `makevp()` helper function. This coerces the data into a data set usable in `ggplot2`, and returns plot objects for DNAm means and variances of probe sets in the `lvp` list. Arrange the plot objects with `grid.arrange`.
