...
First, load the provided data objects. These are required for evaluated code 
chunks below. Note these objects result from running the provided script and 
annotated code.
...
Next, retrieve just the provided sample metadata from the `recountmethylation` 
package files.
...
This vignette also makes use of 2 `HDF5-SummarizedExperiment` databases. 
Consult the `users_guide` vignette for details about the database file 
formats and download instructions. With the datasets downloaded, load them 
