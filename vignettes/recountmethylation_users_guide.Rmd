...
DNAm database files available through `recountmethylation` are also indexed on `ExperimentHub`. You may view the files on the hubs as follows.
...
The `getdb` functions implement caching and are sufficient for both database file types. You may alternatively download the `.h5` files from the hubs, with caching, as follows.

```{r}
eid <- "EH3776" # h5 test file id
rmdat[[eid]] # download with default caching
```
